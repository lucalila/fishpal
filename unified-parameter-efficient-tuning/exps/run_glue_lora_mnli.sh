...
TASK_NAME=mnli  # todo: second file for mnli
metric="accuracy"
...
ffn_adapter_init_option="bert"  # todo: LoRA and adapter employ different parameter initialization methods:
# todo: LoRA uses a random Kaiming uniform (He et al., 2015) initialization for Wdown and zero for Wup (LoRA init),
# todo: while adapters use the same initialization as BERT (Devlin et al., 2019).
# todo: We found it beneficial to use the same initialization method as LoRA in scaled PA.
